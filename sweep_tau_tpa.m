...
%     ph2 = plot(x,feval(fr,x),'--');
    plot(xlim,.25*ones(size(xlim)),'--k');
    plot(xlim,.50*ones(size(xlim)),'--k');
    hold off;
...
%     end
%     title(str);
    title(sprintf('TPA = %G GM', tpa/1e-58));
...
str = sprintf('TPA = %G GM, \\tau = [%s, %s]', tpa/1e-58, tauStr(min(TAU)), tauStr(max(TAU)));
title(str)
axis square
...
fh = fopen(fullfile('FILES', [fileName,'.txt']), 'w');
...
CW_fprintf(fh, '\n');
CW_fprintf(fh, '\tA [W.sqrt(s)]\tTau_0 [s]\tgof\ta [J.m^2]\n');
CW_fprintf(fh, '\t%g\t%g\t%g\t%g\n', A, tau0, gf.rsquare, A*sqrt(tpa));
CW_fprintf(fh, '##################################################################\n')
CW_fprintf(fh, '\n');

fclose(fh);

print(gcf, fullfile('FILES', fileName), '-dpdf');

save(fullfile('FILES', fileName));
